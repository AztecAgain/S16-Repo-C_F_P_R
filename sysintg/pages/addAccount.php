<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			//error checks
			if (empty($username)){
				$message.='<font color="red"><b>ERROR:</b></font> Invalid user name</b><br>';
				$error=true;
			}
			if (empty($password)){
				$message.='<font color="red"><b>ERROR:</b></font> Invalid passsword</b><br>';
				$error=true;
			}
			...
			if(!isset($message)){
				...
				// get max accountID
				$accountID_query="select *
				                    from accounts 
								   order by accountID 
								    desc 
								   limit 1";	
				$accountID_query_result=mysqli_query($dbc,$accountID_query);			
				$data=mysqli_fetch_array($accountID_query_result,MYSQLI_ASSOC);		
				$accountID = intval($data['accountID']) + 1;
				
				// Insert into accounts table
				$insert_query="insert into accounts (accountID,username,password,lastName,firstName) 
						                   values ('{$accountID}','{$username}','{$password}','{$lastName}','{$firstName}')";
										   
				$insert_query_result=mysqli_query($dbc,$insert_query);
						
				if ($insert_query_result) {
					$message="<font color='green'><b>Account Added SUCCESSFULLY!</b></font><br>
								<b>AccountID:</b> $accountID <br>
					...
					$message="<font color='red'><b>Error adding the account</b></font><br>
								<b>AccountID:</b> $accountID <br>
								<b>Username:</b> $username <br>
								<b>Password:</b> $password <br>
								<b>Last Name:</b> $lastName <br>
				...
				<div class="col-lg-12">
					<div class="panel panel-default">
						<div class="panel-heading">
							<i class="fa fa-user fa-fw"></i> Account Form
						</div>
